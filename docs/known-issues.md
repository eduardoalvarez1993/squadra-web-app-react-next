...
| TI-DP-001 | Segurança / Permissão | **Resolvido no código (2026-06-08/09):** `/v1/pessoa/permissoes` passou a honrar `pessoaId` e `perfilDP` popula; o fallback de acesso por cargo foi **removido** (`dp-access.ts` não é mais usado p/ liberar RH — só `perfilDP`). **Pendente só do lado da TI:** marcar a flag `perfilDP` para quem ainda não tem (ex.: Vera). | 2026-06-09 | Baixa |
...
| DEBT-002 | Arquitetura | **Refactors planejados (alto esforço, fazer com calma):** (1) `fetchJson`/`postJson` único em `src/lib/api-client.ts` padronizando extração de `error` — hoje duplicado em ~10 hooks com versões divergentes; (2) `src/lib/format.ts` para data/hora/saldo (5+ versões duplicadas, risco off-by-one UTC com `new Date('YYYY-MM-DD')`); (3) wrapper `withSession()` para o boilerplate de ~50 route handlers; (4) migrar schemas estáveis (Login, Permissoes, Contracheque) de `z.unknown().transform` para `z.object().passthrough()` para detectar regressões de contrato. Não feitos em massa para evitar regressão sem testes. | 2026-06-04 | Média |
| A11Y-PEND | Acessibilidade | Pendentes do platform-review: CropModal (`ProfileForm.tsx`) sem focus-trap/Esc; `<select>`/`<input>` sem `htmlFor`/`id` em percentual e solicitações; empty states inconsistentes (imagem vs texto); áreas de toque < 44px em alguns botões mobile. | 2026-06-04 | Baixa |
